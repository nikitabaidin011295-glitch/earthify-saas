...
const fmtCurr = (n: number) => '₴' + Math.round(n).toLocaleString('uk-UA')
...
const METRICS: MetricDef[] = [
  { key: 'bookingsCount', label: 'Бронювань', format: 'n' },
  { key: 'nights', label: 'Ночей', format: 'n' },
  { key: 'los', label: 'Середня тривалість (LOS)', format: 'd' },
  { key: 'guestsTotal', label: 'Гостей всього', format: 'n' },
  { key: 'occupancy', label: 'Завантаженість', format: 'pct', color: '#534AB7', bold: true },
  { key: 'adr', label: 'ADR (середня ціна/ніч)', format: 'curr', color: '#0F6E56', bold: true },
  { key: 'revpar', label: 'RevPAR', format: 'curr', color: '#D85A30', bold: true },
  { key: 'revenue', label: 'Загальний виторг', format: 'curr', bold: true },
  { key: 'revenueAccommodation', label: '· Проживання', format: 'curr' },
  { key: 'revenueOther', label: '· Інші послуги', format: 'curr' },
  { key: 'clientsNew', label: 'Нових клієнтів', format: 'n' },
  { key: 'clientsRepeat', label: 'Повторних клієнтів', format: 'n' },
]
  ...
  return (
    <div style={{ padding: '1.5rem 2rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 600, margin: 0 }}>📊 Аналітика</h1>
          <p style={{ color: '#888', fontSize: 13, margin: '2px 0 0' }}>Метрики по місяцях: ADR, RevPAR, завантаженість</p>
        </div>
        <div style={{ display: 'flex', gap: 4, background: '#f5f5f7', padding: 4, borderRadius: 10 }}>
          {[{ id: 'table', label: 'Таблиця' }, { id: 'charts', label: 'Графіки' }].map(t => (
      ...
      {totals && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: '1.5rem' }}>
          {[
            { label: 'Завантаженість', value: fmtPct(totals.occupancy), sub: 'Серед. по вибраному', color: '#534AB7', bg: '#EEEDFE' },
            { label: 'ADR', value: fmtCurr(totals.adr), sub: 'Середня ціна/ніч', color: '#0F6E56', bg: '#E1F5EE' },
            { label: 'RevPAR', value: fmtCurr(totals.revpar), sub: 'Виторг на номер', color: '#D85A30', bg: '#FAECE7' },
            { label: 'Загальний виторг', value: fmtCurr(totals.revenue), sub: `${totals.bookingsCount} бронювань`, color: '#185FA5', bg: '#E6F1FB' },
          ].map((s, i) => (
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        ...
        <button onClick={() => setSelectedMonths(DATA.map(d => d.month))} style={{ padding: '5px 12px', borderRadius: 20, border: '1px solid #ddd', fontSize: 12, cursor: 'pointer', color: '#534AB7', background: '#EEEDFE', borderColor: '#534AB7' }}>Всі</button>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 700 }}>
            <thead>
              <tr style={{ background: '#fafafa' }}>
                <th style={{ padding: '11px 14px', textAlign: 'left', fontSize: 11, color: '#aaa', fontWeight: 600, borderBottom: '1px solid #f0f0f0', minWidth: 180 }}>Показник</th>
                {visibleData.map(d => {
                  const [y, m] = d.month.split('-')
                ...
                {totals && <th style={{ padding: '11px 14px', textAlign: 'right', fontSize: 11, color: '#534AB7', fontWeight: 700, borderBottom: '1px solid #f0f0f0', minWidth: 100 }}>Разом/Сер.</th>}
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={metric.key} style={{ borderBottom: '1px solid #f5f5f5', background: metric.bold ? '#fafafe' : 'transparent' }}>
                  ...
                  {totals && (
                    <td style={{ padding: '10px 14px', textAlign: 'right', fontSize: 13, fontWeight: 700, color: metric.color || '#534AB7' }}>
                      {fmtVal(totals[metric.key] as number, metric.format)}
                    </td>
                  )}
                </tr>
              ))}
      ...
      {tab === 'charts' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          ...
          <div style={{ background: '#fff', border: '1px solid #ebebeb', borderRadius: 14, padding: '1.5rem' }}>
            <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: '1.25rem', color: '#111' }}>Виторг по місяцях</h3>
              ...
              {visibleData.map(d => {
                ...
                return (
                  <div key={d.month} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                    <div style={{ fontSize: 9, color: '#888', textAlign: 'center' }}>₴{Math.round(d.revenue / 1000)}к</div>
          ...
          {/* Occupancy chart */}
          <div style={{ background: '#fff', border: '1px solid #ebebeb', borderRadius: 14, padding: '1.5rem' }}>
            <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: '1.25rem', color: '#111' }}>Завантаженість (%)</h3>
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, height: 160 }}>
              {visibleData.map(d => {
                const [y, m] = d.month.split('-')
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 12 }}>
              {[{ color: '#10b981', label: '≥70%' }, { color: '#f59e0b', label: '55–70%' }, { color: '#ef4444', label: '<55%' }].map(l => (
          ...
          <div style={{ background: '#fff', border: '1px solid #ebebeb', borderRadius: 14, padding: '1.5rem' }}>
            <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: '1.25rem', color: '#111' }}>Нові vs повторні клієнти</h3>
                ...
                return (
                  <div key={d.month}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}>
                      ...
                      <span style={{ color: '#888' }}>{d.clientsNew} нових · {d.clientsRepeat} повторних</span>
                    </div>
            ...
            <div style={{ display: 'flex', gap: 16, marginTop: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><div style={{ width: 12, height: 12, borderRadius: 2, background: '#534AB7' }} /><span style={{ fontSize: 11, color: '#888' }}>Нові</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><div style={{ width: 12, height: 12, borderRadius: 2, background: '#f0f0f0' }} /><span style={{ fontSize: 11, color: '#888' }}>Повторні</span></div>
            </div>
          </div>
